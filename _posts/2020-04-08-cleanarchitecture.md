...
*시사점 :시사점 : 오버엔지니어링 보다는 언더엔지니어링 낫다 :) 무턱대고 경계를 만들지 말고 비용 대비 효율을 고려하자!!*
...
*시사점 : cross-cutting concern에 취약한 서비스들에게 그 내부를 컴포넌트 아키텍처로 구성하도록 할 필요가 있는가? 예를 들면 sam, wam, lsm, umediaserver등의 서비스들 내부에 앱과 비디오 라이프싸이클에 관련된 컴포넌트로 경계를 만든다.*
...
데이타 모델은 중요하다. 하지만 데이타베이스는 데이타에 접근하는 유틸리티일뿐이다. 아키텍처 관점에서 보면 이러한 유티리티는 저수준의 세부사항으로 업무 규칙으로 부터 분리되고 의존성 규칙을 따르게 해야 한다.



*시사점 : 시스템 전체적으로 보면 웹 오에스에서의 데이타베이스는 leveldb이다. 그리고, DB8은 Gateway역할을 담당할 것이다.*

*시사점: 단일 서비스 혹은 앱 관점에서 보면 Data Access에 대한  자신만의 Gateway를 두어야 한다.*



## 31장. 웹은 세부사항이다

GUI는 세부사항이다. 웹은 GUI다. 따라서 웹은 세부사항으로 행심 업무 로직에서 분리된 경계 바깥에 두어야 한다.

*시사점: 플랫폼 경계 밖에 앱을 두었다. 하지만, 때때로 플랫폼 내에 앱에 대한 코드 의존성이 있다. (deep linking) 러한 의존성은 테이블화하여 업무 규칙으로 부터 분리해야 한다.*
...
- 프레임워크 제작자는 당신이 풀어야 할 특별한 관심사를 염두해 두지 않는다. 그들은 당신을 알지 못하며, 당신이 풀어야 할 문제도 알지 못한다.
- 당신은 프레임뭐크를 위해 대단히 큰 헌신을 해야 하지만, 프레임워크 제작자는 당신을 위해 아무런 헌신도 하지 않는다.
- 프레임워크 제작자는 당신의 애플리케이션이 가능하면 프레임워크에 공고하게 결합될 것을 강하게 역설한다. 이러한 사용자가 많아질 수록 프레임워크가 효과적임을 검증하는 최고의 방법이기 때문이다.

**위험 요인**

- 프레임워크 제작자는 당신의 업무규칙이 자신의 코드를 상속할 것을 요구한다. 자신의 프레임워크가 당신의 가장 안쪽 원과 결합되기를 원한다. 하지만, 프레임워크가 한번 안으로 들어오면, 다시는 원 밖으로 나오지 않을 것이다.
- 초기에는 프레임워크가 기능을 만드는데 많은 도움이 될 것이다. 하지만 제품이 성숙해지면서 프레임워크가 제공하는 기능과 틀을 벗어나게 될 것이다.
- 프레임워크가 당신에게 도움이 되지 않는 방향으로 진화할수도 있다. 당신은 신규 버전으로 업그레이드하느라 다른 일을 못할 수도 있다. 심지어, 사용중이던 기능이 사라지거나 반영하기 힘든 형태로 변경 될수 있다.
- 새롭고 더 나은 프레임워크가 등장해서 갈아타고 싶을 수도 있다.

**해결책**

* 프레임워크와 결합하지 말고, 그것을 아키텍처의 바깥쪽 원에 속하는 세부사항으로 취급하라.
* 프레임워크가 자신의 기반 클래스로 부터 파생하기를 요구한다면, 거절하라. 대신 프락시를 만들고 업무 규칙에 플러그인할 수 있는 컴포넌트에 이들 프락시를 위치하라.
* 즉, 핵심 코드에 플러그인할 수 있는 컴포넌트에 프레임워크를 통합하고, 의존성 규칙을 준수하라.
* 메인 컴포넌트에서 의존성을 주입하라.

*시사점 : 미디어는 gstreamer와 강하게 결합 되어 있다. 심지어 웹 오에스의 미디어 HAL은 gstreamer plug-in이다. 미래에 좀 더 나은 미디어 프레임워크(일테면 안드로이드 MediaPlayer와 같은)를 도입해보자는 유연한 생각은 할 수가 없다.*
