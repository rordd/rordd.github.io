...
>*시사점 : OCP는 확장가능성이 있는 코드는 열어두고, 경계를 두어 나머지 코드를 변경으로 부터 보호하는 것이다. 그렇다면 확장 가능성은 어떻게 예측 가능한가? 사실상 예측 하기 힘들다. 그러므로 초기 시스템은 최대한 심플하게 만들고, 경험을 통해 확장용이한 구조로 진화시켜야 한다. (즉, 초기부터 확실하지 않는 확장가능한 구조들을 덕지 덕지 만들어 시스템을 불필요하게 복잡하고 무겁게 만들면 안된다.) 그런데, 어떻게 시스템을 심플하게 만들 수 있는가? 가장 유명한 실천법은 TDD이다. 테스트 코드를 먼저 만들고, 딱 그것이 동작하는 수준의 시스템만 만들게 유도하여 오버엔지니어링을 예방한다. 그 외에도 TDD는 테스트 대상 코드를 격리하기 위해 mock을 만들게 되면서, 자연스럽게 코드를 외부로 부터 격리하여 작고 느슨하게 결합된 구조를 유도한다. 즉, 테스트가 아키텍처를 드라이브하여 좋은 설계를 유도한다.*

>*시사점 : 정책과 세부 사항은 컨텍스트에 따라 상대적인 것이다. 테스트의 경우 테스트 코드가 정책이고 gmock은 세부 사항이다. 그러므로, cpptest등과 같은 다른 테스트 프레임워크를 사용하더라도 테스트 코드에 영향 없게 하여 저비용으로 테스트 프레임워크를 교체 가능하도록 설계해야 한다. 그런데, gmock을 어떠한 근거로 기술적 제약사항으로 선언할 수 있다면, 이 또한 오버엔지니어링이 될 것이다. 높은 품질의 제약 사항은 컨선을 격리하여 아키텍처를 단순하게 유지할 수 있는 강력한 툴이다.*
