...
시스템 설계 상의 의사 결정은 트레이드오프(trade-off)에 대한 선택이다.  이러한 설계 의사 결정에 도움을 주는 설계 원칙과 실천 사항을 정의할 필요가 있다. 즉, 설계 의사 결정을 프레이밍(framing) 한다.
...
## 필수 기준 (The Required Standard)

아키텍트는 개별 마이크로 서비스 간에 자율성을 허용하면서도, 바람직한 서비스로서의 일관성을 유지하기 위해서는 각 서비스가 가져야할 공통적인 속성을 정의할 필요가 있다.

#### 모니터링(Monitoring)

서비스간 경계를 넘어 시스템 상태를 일관되게 살펴 볼 수있어야 한다. 서비스 세부 상태 보다는 시스템 전체 상태를 통해 광범위한 문제와 동향을 분석하고 이해할 필요가 있다.

이를 위해서 모든 서비스가 표준적인 방법으로 자신의 상태와 일반적인 모니터링 관련 지표(matrics)를 푸시 하거나 폴링으로 받아오는 모니터링 시스템을 지원한다. (로깅 역시 한 곳에서 수집되도록 해야 한다.)

#### 인터페이스(Interfaces)

서비스 간 인터페이스 기술(Technologies)의 개수를 최소로 유지하는 것이 신규 서비스를 통합하는데 유리하다.

예를 들어 HTTP/REST를 택한다면 동사형과 명사형 중 하나를 선택하고, 리소스의 페이지 나눔(pagination)처리 방법을 선택하고, 엔드포인트(endpoint) 버전을 구분하는 방법을 고려해야 한다.

#### 아키텍처 안정성 (Architectural Safety)

아키텍트는 특정 서비스의 오동작이 시스템 전체를 망가뜨리게 해서는 안된다. 서비스들이 비정상적인 호출로 부터 자신을 잘 보호하도록 해야 한다. 

- 요청한 응답이 지연되거나 오류가 발생하는 경우 빠르게 회로차단기(circuit breaker)를 사용해서 차단한다.
- 규약대로 응답 코드(정상, 오류 등)를 반환하도록 해야 한다.
- 잘못된 요청이나 응답을 빠르게 실패시켜 문제를 추적하도록 해야 한다.
